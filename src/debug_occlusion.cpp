...
int main() {
    std::vector<std::string> names = {
        "35_(np.int32(457), np.int32(103))_area(1505.5)_state(0)",
        "35_(np.int32(461), np.int32(209))_area(983.5)_state(0)",
        "42_(np.int32(100), np.int32(200))_area(500.0)_state(1)",
        "42_(np.int32(100), np.int32(200))_area(500.0)_state(1)_m"
    };

    // 方法1：使用正则表达式提取后缀
    std::regex pattern(R"(_state\(\d+\)[^\)]*$)"); // 匹配从_state(数字)开始到结尾的部分
    
    std::set<std::string> unique_suffixes;
    std::string tmp_match = "ok";
    for (const auto& name : names) {
        std::smatch match;
        if (std::regex_search(name, match, pattern)) {
            unique_suffixes.insert(match[0]);
            std::cout << "match: " << match[0] << "\n";
            if (tmp_match == match[0]) {
                std::cout << "same" << "\n";
            } else {
                tmp_match = match[0];
            }
    ...
    std::cout << "使用正则表达式提取结果:\n";
    std::cout << "找到 " << unique_suffixes.size() << " 种不同的后缀格式:\n";
    ...
    for (const auto& name : names) {
        size_t pos = name.find("_state");
        if (pos != std::string::npos) {
            suffix_set.insert(name.substr(pos));
        }
    }

    std::cout << "\n使用字符串查找提取结果:\n";
    std::cout << "找到 " << suffix_set.size() << " 种不同的后缀格式:\n";
    for (const auto& suffix : suffix_set) {
        std::cout << "- " << suffix << "\n";
    }

    // 检查所有后缀是否相同
    if (suffix_set.size() == 1) {
        std::cout << "\n所有字符串的后缀完全相同\n";
    } else {
        std::cout << "\n字符串后缀存在差异\n";
        ...
        for (const auto& name : names) {
            ...
            if (pos != std::string::npos) {
                ...
                grouped[suffix].push_back(name);
            }
        }
        
        std::cout << "\n分组结果:\n";
        for (const auto& [suffix, items] : grouped) {
            std::cout << "\n后缀 '" << suffix << "' 的字符串:\n";
